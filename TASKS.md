...
## [x] BE-025 — مدل کالای زیورآلات

> **وضعیت:** ۲۳ تست e2e روی PostgreSQL واقعی + ۲۹ تست قرارداد + ۱۴ تست
> `core-calc`. وزن خالص ستون نیست و از `articlePureMg` می‌آید؛ کسورات
> **پیش از** عیار اعمال می‌شوند و تستی نشان می‌دهد ترتیب برعکس چه خطایی
> می‌سازد. مقدار اعشاری وزن در قرارداد رد می‌شود، نه گرد.
>
> ضمناً دروازه‌ی پوشش `core-calc` که از BE-020 قرمز مانده بود (شاخه‌ی
> منفیِ `grossUg`) درست شد — حالا ۱۰۰٪ در هر چهار سنجه.
...
## [x] BE-026 — API کالای زیورآلات

> **وضعیت:** ۵۲ تست e2e روی PostgreSQL واقعی + ۲۵ تست قرارداد.
>
> **تصمیم مدل درباره‌ی عنوان:** تغییر وزن، عیار یا اجرت نسخه‌ی جدید
> می‌سازد؛ تغییر عنوان روی همان نسخه‌ی **باز** می‌نشیند و شماره‌ی نسخه را
> جلو نمی‌برد. عنوان روی هیچ عددی اثر ندارد و فاکتور هم مشخصات لحظه‌ی
> فروش را snapshot می‌کند، پس نسخه‌ی جدید فقط تاریخچه را با ردیف‌هایی پر
> می‌کرد که هیچ‌کدام مبلغ متفاوتی نمی‌سازند. نسخه‌های **بسته** در هیچ
> حالتی ویرایش نمی‌شوند.
>
> غیرفعال‌سازی هم وضعیت است نه مشخصه‌ی مالی: روی نسخه‌ی باز می‌نشیند،
> نسخه نمی‌سازد، و `requireSelectableForSale` تنها دروازه‌ی انتخاب کالا در
> سند جدید است تا ماژول فروش نتواند از کنارش رد شود.
>
> برای جست‌وجوی فارسیِ عنوان، ستون `normalized_title` (مهاجرت ۰۰۱۴) به
> `jewelry_item_versions` اضافه شد — همان الگوی `parties.normalized_name`.
> جایگزینش بازنویسی قواعد نرمال‌سازی `core-calc` داخل SQL بود.
>
> ⚠️ «فیلتر موجود/ناموجود» به فعال/غیرفعال نگاشت شده است؛ موجودی واقعی
> جدول ندارد تا BE-027 (حرکات موجودی).
...
## [x] BE-027 — مدل حرکات موجودی

> **وضعیت:** ۳۲ تست e2e روی PostgreSQL واقعی + ۱۲ تست قرارداد.
>
> **واحد `quantity` به `item_type` بستگی دارد:** زیورآلات و سکه تعداد
> (با `item_id`)، آبشده میلی‌گرم طلای خالص ۱۰۰۰ (بدون `item_id`، چون
> هویت مستقلی ندارد). ستون یکی است و `bigint` — همان تصمیمی که BE-032
> برای `ledger_entries` می‌گیرد. علامت جهت را می‌سازد: ورودی مثبت،
> خروجی منفی. موجودی هیچ‌جا cache نمی‌شود.
>
> **سیاست موجودی منفی:** مسدود است، با کلید نسخه‌دار
> `inventory.allow_negative_stock` (پیش‌فرض `false`) در
> `INITIAL_TENANT_SETTINGS` — قاعده‌ی ۲-۶. نبودِ تنظیم خطای صریح می‌دهد و
> پیش‌فرض کدی جایش را نمی‌گیرد. بررسی پشت قفل advisory روی
> «مستأجر + نوع + کالا» است تا دو فروش هم‌زمان با هم موجودی را منفی نکنند.
>
> **Append-Only:** تریگر مهاجرت ۰۰۱۵ جلوی UPDATE/DELETE نقش `gold_app` را
> می‌گیرد (همان الگوی `price_quotes`)، و سرویس هیچ متد نوشتنیِ دیگری
> ندارد. اصلاح = حرکت جدید با علامت مخالف و `source_type = 'CORRECTION'`.
>
> ⚠️ **دو انحراف آگاهانه از متن تسک، چون وابستگی‌شان هنوز ساخته نشده:**
> `dimension_id` فعلاً `nullable` و بدون کلید خارجی است — `asset_dimensions`
> در BE-030 می‌آید و کلید خارجی و `NOT NULL` همان‌جا اضافه می‌شوند.
> قاعده‌ی «movement و ledger در یک transaction» هم هنوز قابل اجرا نیست
> (دفتر کل در BE-032)؛ به‌جایش تنها مسیر نوشتن `recordInTransaction` است
> که تراکنش را از فراخوان می‌گیرد، و تستی نشان می‌دهد rollback فراخوان
> حرکت را هم برمی‌گرداند.
...
## [x] BE-028 — موجودی اولیه

> **وضعیت: completed (2026-08-06).** پس از BE-029 تا BE-034، سند افتتاحیه،
> حرکت‌های append-only موجودی و posting متوازن دفتر کل در همان PostgreSQL
> transaction و زیر Idempotency-Key ثبت می‌شوند.
...
## [x] BE-032 — مدل تراکنش و Entry دفتر کل
...
## [x] BE-038 — شماره فاکتور بدون شکاف

> **وضعیت:** `DocumentCountersService` دقیقاً طبق الگوریتم تسک —
> `INSERT ... ON CONFLICT DO NOTHING` فقط ردیف شمارنده را می‌سازد (بدون
> شرط رقابتی)، بعد `SELECT ... FOR UPDATE` قفل می‌گیرد، بعد `UPDATE`
> مصرف می‌کند؛ هیچ `SEQUENCE` در کار نیست. متد `TenantTransaction`
> فراخوان را می‌گیرد، خودش تراکنش تازه باز نمی‌کند، پس rollback فراخوان
> خودِ افزایش را هم برمی‌گرداند — تست شده. استراتژی دوره‌ای فاز ۱: سال
> جلالی، با `Intl` بومی نه `date-fns-jalali` (همان تصمیمی که وب برای
> نمایش گرفت — `apps/web/src/lib/date.ts`)، مرز نوروز تست شده. تست
> concurrency واقعی: ۲۰ درخواست هم‌زمان دقیقاً {۱..۲۰} می‌گیرند، بدون
> تکرار و بدون شکاف.
...
## [x] BE-039 — مدل فاکتور فروش نسخه‌دار
...
## [x] BE-052a — پیش‌نمایش فقط‌خواندنی Buyback B2C

**هدف** — فراهم‌کردن breakdown قابل‌نمایش پیش از ثبت خرید، بدون ساخت هیچ
سند حسابداری یا تغییر موجودی.

**endpoint**

```http
POST /sales/invoices/:invoiceId/b2c-buyback/preview
```

**رفتار**

- ورودی فقط اندازه‌گیری امروز، عیار خرید، مظنه و زمان رویداد است؛ مبلغ یا طرف
  حسابِ فروش قبلی از کلاینت پذیرفته نمی‌شود.
- پاسخ، breakdown محاسبه‌شده در سرور و نرخ/تاریخ قفل‌شدهٔ فروش اولیه و خرید
  امروز را جداگانه بازمی‌گرداند.
- هیچ `second_hand_purchase`، سند دفترکل، حرکت موجودی، audit یا رکورد
  idempotency ایجاد نمی‌شود.

**تمام است وقتی**

- پیش‌نمایش با محاسبهٔ ثبت‌شده هم‌مسیر است.
- فاکتور اصلی و همهٔ اثرهای حسابداری/موجودی دست‌نخورده می‌مانند.

> تکمیل BE-052a: endpoint فقط‌خواندنی preview با استفاده از همان قیمت‌گذاری
> نسخه‌دار مسیر ثبت افزوده شد. پاسخ breakdown سرور و snapshot نرخ/تاریخ هر دو
> رویداد را دارد؛ آزمون e2e نبود هرگونه اثر جانبی را بررسی می‌کند.
