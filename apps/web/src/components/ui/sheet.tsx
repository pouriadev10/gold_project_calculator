  ...
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        ...
        // نام‌گذاری منطقی، نه چپ/راست: در RTL خودبه‌خود قرینه می‌شود
        start:
...
>(({ side = "end", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      <SheetPrimitive.Close className="absolute end-4 top-4 grid size-touch cursor-pointer place-items-center rounded-sm text-muted-foreground ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
        <X className="size-5" />
